...
static void lvgl_touch_cb(lv_indev_t *indev, lv_indev_data_t *data)
{
  ...
  {
    // require a few consecutive misses before treating as a real release,
    // to survive transient I2C read races on the FT3168
    if (miss_count < 2)
    {
      miss_count++;
      data->point.x = last_x;
      data->point.y = last_y;
      data->state = LV_INDEV_STATE_PRESSED; // pretend still pressed
    }
    else
    {
